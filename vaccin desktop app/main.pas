...
val :=QuotedStr('%'+ComboBox1.Text+'%');
  if (combobox1.Text = '«·ﬂ·') or (combobox1.Text = '') then
  ...
  label4.Caption:=' ÂÊ :  '+inttostr(mdata.data.child.recordcount);
  if (combobox1.ItemIndex = 0) or (combobox1.ItemIndex = -1) then
    label5.Caption:='Ã„Ì⁄ «·”‰Ê« '
  else
...
var val:string;
begin
  if ((combobox2.Text = '«·ﬂ·') or (combobox2.Text = ''))
  and ((combobox1.Text = '«·ﬂ·') or (combobox1.Text = '')) then
  mdata.data.child.Filtered:= false
  else
  begin
  if (combobox2.Text = 'œ«Œ· «·»·œÌ…') then
   begin
    if (combobox1.Text = '«·ﬂ·') or (combobox1.Text = '') then
     begin
      val:='‰⁄„';
